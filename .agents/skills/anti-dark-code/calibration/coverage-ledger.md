...
Freshness: reviewed at commit `2a8df3e3` on 2026-08-09; diff-refreshed at `6036896b` on 2026-08-14.
...
| Linux SG_IO transport and rip extraction | 2026-08-14 | live hardware on 3 drives plus Windows CI | INQUIRY/TOC/READ CD/C2/secure windows/calibration/cache defeat/full Test & Copy byte-exact against AccurateRip and CTDB; two firmware quirks characterized by probe matrix; cross-process lease hole found and fixed with a live two-process denial test | Linux-head suite (49) plus fork CI lanes; probe tools are Debug-only | transport, lease, eviction, or carve-out changes | current at the rip-sprint head |
| App.Core extraction wave (convert/queue/settings/rip/art, ~7k lines) | 2026-08-14 | move-with-namespaces plus both heads' CI | file moves kept namespaces and test standing; source-inspection tests required a path-literal sweep (twice) | Wpf.Tests (512) and Linux suite; DeadSwitchAnalyzer knows App.Core | further moves or namespace renames | current |
