...
namespace TTA {
	...
	public:
		AudioEncoder(EncoderSettings^ settings, String^ path, Stream^ IO)
		{
		    ...
		    _outputTransaction = gcnew LosslessFileOutputTransaction(path);
		    _verificationFingerprint = gcnew LosslessPcmFingerprint();
		    _initialized = false;
		    _closed = false;
		    _ttaWriter = nullptr;
		    _IO = nullptr;
		...
		virtual void Close()
		{
			...
			try
			{
				_outputTransaction->Complete(
					gcnew LosslessOutputOperation(
						this,
						&AudioEncoder::FinalizeAndVerify));
			}
			finally
			{
				ReleaseEncoderAndStreamNoThrow();
				if (_verificationFingerprint != nullptr)
				{
					delete _verificationFingerprint;
					_verificationFingerprint = nullptr;
				}
			}
		}
		...
		virtual property Int64 FinalSampleCount {
			...
			void set(Int64 value) {
				if (_closed) {
					throw gcnew InvalidOperationException("The encoder is already closed.");
				}
				if (value < 0) {
				...
				if (value > UInt32::MaxValue) {
					throw gcnew ArgumentOutOfRangeException(
						"FinalSampleCount",
						"TTA1 supports at most 2^32-1 samples.");
				}
				if (_initialized) {
		...
		virtual void Write(AudioBuffer^ sampleBuffer) {
			...
			try
			{
				accepted = _ttaWriter->CompressBlock(buffer, sampleBuffer->Length);
			} catch (TTALib::TTAException ex)
			{
				throw gcnew Exception(String::Format("TTA encoder: {0}", gcnew String(TTAErrorsStr[ex.GetErrNo()])));
			}
			if (!accepted)
				throw gcnew Exception("TTA encoder accepted no samples.");

			_verificationFingerprint->Append(sampleBuffer);
	...
	private:
		...
		bool _initialized, _closed;
		String^ _path;
		Int64 _finalSampleCount, _samplesWritten;
		EncoderSettings^ _settings;
		LosslessPcmFingerprint^ _verificationFingerprint;
		LosslessFileOutputTransaction^ _outputTransaction;

		void FinalizeAndVerify()
		{
			FinalizeEncoder();

			array<Byte>^ expectedDigest =
				_verificationFingerprint->Complete();
			// TTA1 stores channel count and interleaved channel order, but no speaker mask.
			// Normalize that unrepresentable metadata while still comparing every PCM byte.
			AudioPCMConfig^ verificationPcm = gcnew AudioPCMConfig(
				_settings->PCM->BitsPerSample,
				_settings->PCM->ChannelCount,
				_settings->PCM->SampleRate,
			...
			LosslessPcmVerifier::Verify(
				...
				_samplesWritten,
				expectedDigest,
				gcnew LosslessAudioSourceFactory(
					this,
					&AudioEncoder::OpenVerificationDecoder));
		}

		IAudioSource^ OpenVerificationDecoder()
		{
			return gcnew AudioDecoder(
				gcnew DecoderSettings(),
				_outputTransaction->WorkPath,
				nullptr);
		}

		void FinalizeEncoder()
		{
			Exception^ failure = nullptr;
			if (_ttaWriter != nullptr)
			{
				try
				{
					delete _ttaWriter;
				...
				{
					failure = gcnew Exception(
						String::Format(
							"TTA encoder: {0}",
							gcnew String(TTAErrorsStr[ex.GetErrNo()])));
				}
				_ttaWriter = nullptr;
			}
			...
			if ((_finalSampleCount != 0) &&
				(_samplesWritten != _finalSampleCount))
			{
		...
		void ReleaseEncoderAndStreamNoThrow()
		{
			if (_ttaWriter != nullptr)
			{
				try
		...
		void Initialize()
		{
			...
			try
			{
				_IO = gcnew FileStream(
					_outputTransaction->WorkPath,
					FileMode::CreateNew,
					FileAccess::ReadWrite,
					FileShare::Read);
			    _ttaWriter = new TTALib::TTAWriter(
					(HANDLE)_IO->Handle,
					0,
					WAVE_FORMAT_PCM,
					_settings->PCM->ChannelCount,
					_settings->PCM->BitsPerSample,
					_settings->PCM->SampleRate,
					static_cast<unsigned long>(_finalSampleCount));
			} catch (TTALib::TTAException ex)
			{
				ReleaseEncoderAndStreamNoThrow();
				try
