...
- Prebuilt binaries vendored in `ThirdParty/`: `CSScriptLibrary.v1.1.dll`, `CUDA.NET.dll`, `Freedb.dll`, `MusicBrainz.dll`, `ICSharpCode.SharpZipLib.dll`, plus `Win32/` and `x64/` (ffmpeg etc.). Note `Freedb.dll` and `MusicBrainz.dll` shadow same-named source projects in this repo. (verified presence; provenance unknown)
