...
| S13 Build/release | `.github/`, `collect_files*.bat`, patches, sln | high | mapped | owned-risky, external-control-plane | workflows read; batch read; patches applied cleanly | 11 (CI test step), 10 |
| S14 Tests | `CUETools/CUETools.Test*/`, `TestRipper/`, `CUETools.TestHelpers/` | medium | mapped | owned-clear | Suites executed 2026-07-02: TestParity 18/18 green, TestCodecs 34/34 green (needs native x64 libFLAC), TestProcessor blocked on never-committed fixtures, TestRipper needs hardware. See `docs/unknowns/coverage-pass.md` | 11 Step 2 (MSTest v2 migration, CI test step, synthetic fixtures) |
