...
| S9 Classic GUI applications | `CUETools/`, `CUERipper/`, `CUEPlayer/`, `CUETools.eac3ui/`, controls | medium | reviewed, commented, tested | owned-risky / legacy-unclear | verified: MOTD is bounded HTTPS text with no image cache; source handlers surface proxy/Icecast save failures; BluTools is the SDK-style GUI pilot with exact API/field/method/PE/config/image-resource preservation and old/new live WPF startup | remaining classic SDK conversions, real CUEPlayer settings persistence/migration, plus full interactive GUI, accessibility, and localization behavior are not covered |
