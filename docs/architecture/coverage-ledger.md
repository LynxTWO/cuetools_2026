...
| S2 Ripping and SCSI | `CUETools.Ripper*/`, `Bwg.*/` | high | commented, tested | legacy-unclear / hardware boundary | verified: C2 accumulation/voting invariants reviewed; modern ripper suite 18/18 passed; payload medium errors enter the existing untrusted-sector recovery path while tested device/transport failures remain fatal; transition retry, batch decomposition, and corroborated nested pinpoint recovery are separately bounded to observed `IllegalRequest 24/00` states; exact child sense/context is preserved; SCSI builds pass for net8/net47/net20; H: and K: completed full-disc reads and simultaneous inquiry/TOC; H: completed a full rip and two-read Test & Copy; K: completed Test and published every Test CRC before a later Copy failure; bounded K: probes passed the failing sector and actual window/speed | repeat damaged K: beyond the nested pinpoint boundary at relative sector 283328; broader drive, firmware, C2, cancellation, disagreement, and media-error matrices remain |
