...
Current-state refresh: 2026-07-28. This ledger records review depth, not a claim
...
| S7 Native and mixed-mode codec wrappers | `CUETools.Codecs.lib*/`, `CUETools.Codecs.MACLib/`, `.ffmpeg/`, `.lame_enc/`, `CUETools.Codecs.TTA/`, `ttalib-1.1/` | high | reviewed, commented, tested | owned-risky / cross-repo-boundary | verified: libFLAC/WavPack/HDCD/LAME partial-init ownership and callback lifetimes, selected staged output ownership, 10 focused lifetime tests, release native probes, source-built TTA archive/import comparison, and explicit legacy HDCD/LAME retain gates; official libhdcd v1.4 built x86/x64 but failed the six-vector behavioral replacement gate while its non-HDCD control matched exactly | remaining wrappers and the full live native matrix are incomplete |
| S8 GPU codecs and parity | `CUETools.Codecs.FLACCL/`, `CUETools.FLACCL.cmd/`, `CUETools.CLParity/` | medium | mapped, tested | owned-risky / dead-code boundary | verified: FLACCL is an optional current path; its SDK-style net47 plugin/command pair preserves the qualified 32-bit host and passed corrected verification on an RTX 3060 across modes 0-8, two CPU workers, 24-bit input, and the exact-frame boundary; CLParity is not current because its registration is commented, it has no consumer or release entry, and its project references a missing binary | remove or deliberately restore CLParity; cross-vendor/device/driver FLACCL behavior remains unknown |
