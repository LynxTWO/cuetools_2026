...
| S2 Ripping and SCSI | `CUETools.Ripper*/`, `Bwg.*/` | high | commented, tested | legacy-unclear / hardware boundary | verified: C2 accumulation/voting invariants reviewed; modern ripper suite 14/14 passed; payload medium errors now enter the existing untrusted-sector recovery path while tested device/transport failures remain fatal; a transition-bound retry is limited to the ASUS-observed `IllegalRequest 24/00` after an accepted speed change; SCSI builds pass for net8/net47/net20; H: and K: completed full-disc reads and simultaneous inquiry/TOC; H: completed a full rip and two-read Test & Copy | repeat the damaged K: run beyond both its former 18-28 second `INVALID FIELD IN CDB` rejection and later `NO SEEK COMPLETE` abort; broader drive, firmware, C2, cancellation, disagreement, and media-error matrices remain |
...
- 2026-07-27: recorded the 14/14 damaged-media/control-transition policy suite, all
