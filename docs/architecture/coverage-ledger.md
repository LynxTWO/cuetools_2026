...
| S9 Classic GUI applications | `CUETools/`, `CUERipper/`, `CUEPlayer/`, `CUETools.eac3ui/`, controls | medium | reviewed, commented, tested | owned-risky / legacy-unclear | verified: MOTD is bounded HTTPS text with no image cache; source handlers surface proxy/Icecast save failures; BluTools is the first SDK-style GUI pilot; CUERipper and ProgressODoom preserve managed/PE/config/localization/decoded-image contracts and old/new live main-form parity | CUETools/CUEPlayer SDK conversions, real CUEPlayer settings persistence/migration, plus full interactive GUI, accessibility, and localization behavior are not covered |
