...
| SC9 untested/tampered release | 2 unsigned evidence / 1 publisher identity | Canonical suite selection, artifact contracts, hashes, SPDX/CycloneDX, immutable actions, and actionlint are enforced. Source-bound hosted classic/WPF CI discovered 630 tests, passed 623, retained seven declared skips, and had zero failures; the unsigned release run and its independently downloaded classic 97-file/WPF 557-file payloads passed artifact, SHA-256, provenance, native-closure, and SBOM validation with zero annotations. The production signing policy selects 117 publisher files and refuses unsigned tag releases, but public publisher identity remains unproven until the protected environment receives a public-trust certificate and a signed tag is independently verified. |
