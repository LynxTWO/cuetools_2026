...
2. **Distill before bytes move.** Extract claims and regeneration recipes (seeds, parameters, exact commands, and measured time or resource use when available) into the manifest while the originals are still in place. Mark unmeasured regeneration effort `unknown`.
...
Before any recursive cleanup, resolve the exact target and every existing parent
component. Reject symlink, junction, mount-point, or other reparse-point targets and
ancestors unless that traversal was separately named and approved. Then walk every
existing descendant without following links and refuse the recursive operation if
any descendant is a reparse point. This full-tree precondition matters on runtimes
whose recursive delete may traverse a nested junction. Lexical containment and a
safe-looking leaf prefix do not make a recursive operation safe.

Keep evidence output strictly outside the artifact it describes: reject both equality
and descendant paths. Validate caller-supplied artifact names as one filename, not a
relative path, before deriving receipt, archive, or manifest paths. Check each final
evidence leaf again immediately before creating or replacing it; a safe output
directory does not make a pre-existing receipt symlink or junction safe.

Source provenance and build state are different records. A dirty submodule may contain
both patch-created project/source files and untracked `.obj`, `.pdb`, `.tlog`, import
libraries, recipes, or other intermediates. Hash the source inputs; classify and
exclude known generated intermediates from the source file list, and report the
classification/count explicitly. Never silently omit a reparse entry or follow a
reparse directory during artifact hashing: fail closed.
...
- Recursive cleanup targets, existing ancestors, and every existing descendant were
  checked for reparse points immediately before mutation.
- Evidence destinations are neither equal to nor contained by the artifact, and
  derived filenames cannot escape through caller-controlled names; each destination
  leaf is rechecked immediately before writing.
- Provenance distinguishes untracked source inputs from generated build state; neither
  category is silently dropped or mislabeled.
