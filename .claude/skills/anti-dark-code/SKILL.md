...
- Trace classifier ancestry, not only the final exception or fallback. If several parent branches can reach the same child operation, log and deterministically test every reachable parent/child route; one covered route does not validate its siblings.
- Treat retry, reset, and fallback scope as part of operation identity. State whether a budget is global, per job, per address, per payload shape, or per exact command; test both exhaustion and reset at the intended boundary so one sibling cannot silently consume another's recovery.
- Separate outcome proof from branch-activation proof. A passing end-to-end scenario does not exercise an intermittent recovery branch unless a counter or trace records that branch; keep its coverage unknown when activation evidence is zero.
- For code-drawn, GPU-composited, or animated UI, lock the semantic input/state contract independently of rendering. Require a deterministic offscreen state matrix, an actual themed-window capture, and a post-warmup allocation bound for hot render loops. Treat A/B compositing mismatches as architecture evidence and fix the seam before tuning colors.
