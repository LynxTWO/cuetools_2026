...
Repo-local lessons proposed for the shared Anti-Dark-Code skill. This
repo's skill copy predates the calibration overlay (no repo-binding.json),
so the deterministic flowback stager cannot run until pass 13 modernizes
the install; entries here are staged for human review in the meantime.

## ADC-LOCAL-001: OS-enforced exclusion semantics do not port
...
## ADC-LOCAL-002: build-mode flags change dev-build runtime capabilities
...
## ADC-LOCAL-003: characterize hardware quirks before classifying them
...
## ADC-LOCAL-004: full-cmdline process matching kills the agent's own shell
...
## ADC-LOCAL-005: tests that read source by path join the move checklist
...
## ADC-LOCAL-006: history rewrites are receipt events
